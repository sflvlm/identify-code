...
def fc_model(image):
    """
    进行预测结果
    :param image: 100图片特征值[100, 30, 80, 3]
    :return: y_predict预测值[100, 4 * 26]
    """
    with tf.variable_scope("model"):
        # 将图片数据形状转换成二维的形状
        image_reshape = tf.reshape(image, [-1, 30 * 80 * 3])
        print(image_reshape.shape)
        # 1、随机初始化权重偏置
        # matrix[100, 30 * 80 * 3] * [30 * 80 * 3, 4 * 26] + [104] = [100, 4 * 26]
        weights = weight_variables([30 * 80 * 3, 4 * 26])
